            ...
            <div key={date} className="mb-5">
              <div className="bg-gray-50 border border-gray-200 rounded-xl px-4 py-2.5 mb-2 flex justify-between items-center flex-wrap gap-2">
                ...
                <div className="flex items-center gap-3 text-sm">
                  <span className="text-gray-400 text-xs">{grp.length} захиалга</span>
                  <span className="font-semibold text-emerald-700">{fmt(dayNet)}₮</span>
                  {dayDelv>0&&<span className="text-xs text-gray-400">−{fmt(dayDelv)}₮ хүргэлт</span>}
                </div>
              </div>
                    ...
                    <div key={o.id} className={`rounded-xl border p-3.5 ${o.status==='delivered'?'border-emerald-100 bg-emerald-50/30':'border-gray-100 bg-white'}`}>
                      <div className="flex justify-between items-start gap-2 flex-wrap">
                        <div className="flex items-center gap-2 flex-wrap">
                          {/* Утас — дарж copy */}
                          <button onClick={()=>copy(o.phone,()=>showFlash('Утас хуулагдлаа ✓'))}
                            className="font-semibold text-gray-800 text-sm hover:text-emerald-600 transition-colors" title="Дарж хуулах">
                            {o.phone}
                          </button>
                        </div>
                        <div className="flex items-center gap-1.5 flex-wrap">
                          ...
                          <button onClick={()=>{ setEditOrder(o);setEditPhone(o.phone);setEditAddr(o.address);setEditDate(o.date);setEditStatus(o.status);setEditDelv(String(o.delivery_fee||'')) }}
                      ...
                      {/* Хаяг — дарж copy */}
                      <button onClick={()=>copy(o.address,()=>showFlash('Хаяг хуулагдлаа ✓'))}
                        className="text-xs text-gray-500 mt-1.5 hover:text-emerald-600 transition-colors text-left w-full" title="Дарж хуулах">
                        {o.address}
                      </button>
                      <div className="text-xs text-gray-400 mt-1">{itemsStr}</div>
