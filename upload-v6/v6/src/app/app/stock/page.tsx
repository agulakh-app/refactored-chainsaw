...
export default function StockPage() {
  ...
  const load = useCallback(async () => {
    ...
    const [{ data: prods },{ data: ls }] = await Promise.all([
      ...
      // Only show 'in' type AND manual adjustments (out from restock, not from orders)
      supabase.from('restock_log').select('*').eq('user_id',user.id)
        .neq('note','Захиалга')  // hide order deductions
  ...
  async function addRestock() {
    ...
    showFlash(p.name+(isNeg?`: −${absQty} хасагдлаа`:`+${qty} нэмэгдлээ`)+' ✓')
    load()
  }
  ...
  async function addNewProduct() {
    ...
    const { data: prod } = await supabase.from('products').insert({
      user_id:user!.id, name:nName.trim(), unit_price:Number(nPrice)||0, stock:Number(nQty)||0, added_date:nDate
    }).select().single()
    ...
    <div className="space-y-5">
      ...
      <div className="card">
        ...
        <div className="bg-gray-50 rounded-xl p-4 mb-5 border border-gray-100">
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            ...
            <div>
              <label className="block text-xs text-gray-500 mb-1">
                Тоо ширхэг
                <span className="text-gray-400 ml-1">(-1 гэж бичвэл хасна)</span>
              </label>
              <input type="number" className={`w-full px-3 py-2 rounded-lg border text-sm ${Number(rQty)<0?'border-red-300 bg-red-50 text-red-700':'border-gray-200'}`}
                value={rQty} onChange={e=>setRQty(e.target.value)} /></div>
            <div><label className="block text-xs text-gray-500 mb-1">Огноо</label>
          ...
          {Number(rQty)<0&&<div className="mt-2 bg-red-50 border border-red-100 rounded-lg px-3 py-2 text-xs text-red-600">⚠ {Math.abs(Number(rQty))} ширхэг агуулахаас хасагдана</div>}
          <div className="flex justify-end mt-3">
            <button onClick={addRestock} className={`px-5 py-2 rounded-xl text-sm font-medium text-white ${Number(rQty)<0?'bg-red-500 hover:bg-red-600':'bg-emerald-600 hover:bg-emerald-700'}`}>
          ...
          return (
            <div key={date} className="mb-4">
              <div className="flex justify-between items-center bg-emerald-50 border border-emerald-100 rounded-lg px-3 py-2 mb-1.5">
                <span className="text-sm font-bold text-emerald-800">{fmtDate(date)}</span>
                ...
                <div key={r.id} className="flex justify-between items-center py-2.5 px-2 border-b border-gray-50 last:border-0 hover:bg-gray-50 rounded-lg group">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button onClick={()=>{ setEditLog(r);setEditQty(String(r.quantity));setEditDate(r.date);setEditNote(r.note||'') }}
                      className="opacity-0 group-hover:opacity-100 p-1.5 rounded-lg hover:bg-blue-50 text-blue-500 transition-all">✏️</button>
                    <button onClick={()=>deleteLog(r)}
                      className="opacity-0 group-hover:opacity-100 p-1.5 rounded-lg hover:bg-red-50 text-red-400 transition-all">🗑️</button>
                  </div>
                </div>
              ))}
            </div>
